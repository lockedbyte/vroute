...
ssize_t http_write_all(SOCKET sock, SSL *c_ssl, char **data, size_t *data_sz, int is_https) {
    int r = 0;
    size_t sent = 0;
    char *ptr = NULL;
    size_t sz = 0;
...
ssize_t http_read_all(SOCKET sock, SSL *c_ssl, char **data, size_t *data_sz, int is_https) {
...
void http_close(SOCKET sock, SSL *c_ssl, int is_https) {
    if(is_https) {
        if(c_ssl) {
            SSL_free(c_ssl);
            c_ssl = NULL;
...
ssize_t ctl_send_data(SOCKET sock, char *host, int port, char **data, size_t *data_size, proto_t proto) {
    ssize_t r = 0;
    char *data_x = NULL;
...
ssize_t ctl_recv_data(SOCKET sock, char *host, int port, char **data, size_t *data_size, proto_t proto) {
    ssize_t r = 0;
    char *data_x = NULL;
    size_t data_size_x = 0;
...
void close_channel_conn(SOCKET sock) {
    if(sock == -1)
        return;
...
int get_channel_id_by_sock(SOCKET sock) {
    int idx = -1;
    int channel_id = -1;
...
int relay_data(SOCKET sock, char *data, size_t data_sz) {
    int r = 0;

    if(sock == -1 || !data || data_sz == 0)
        return 0;

    r = send(sock, data, data_sz, 0);
    if(r < 0) {
...
uint32_t do_tcp_handshake(SOCKET sock, char *key, size_t key_sz) {
    int r = 0;
    size_t out_size = 0;
    char *p = NULL;
    uint32_t cid = 0;
    ...
    r = recv(sock, chall, CHALLENGE_DEFAULT_SIZE, 0);
    if(r <= 0) {
        VR_LOG(LOG_ERROR, "Error receiving challenge from control server: %d: %s", sock, strerror(errno));
        return 0;
    }
    ...
    r = recv(sock, &cid, sizeof(uint32_t), 0);
...
uint32_t handshake(SOCKET sock, char *host, int port, proto_t proto, char *key, size_t key_sz) {
    uint32_t cid = 0;
...
int proper_select(int largestFileDescriptorValuePlusOne, struct fd_set * readFS, struct fd_set * writeFS, struct fd_set * exceptFS, struct timeval * timeout)
{
   ...
   return select(largestFileDescriptorValuePlusOne, readFS, writeFS, exceptFS, timeout);
}
...
int fake_read(int fd, void *buf, size_t count)
{
    ...
    DWORD bytes_read;
    BOOL success = ReadFile(handle, buf, count, &bytes_read, NULL);
    if (success) {
        return bytes_read;
    } else {
        return -1;
...
void relay_poll(char *host, int port, proto_t proto) {
    ...
    while(1) {
        ...
        for(int x = 0 ; x < MAX_CONCURRENT_CHANNELS ; x++) {
            if(pending_ch_array[x] != 0 && pending_ch_array[x] > 0) {
               ...
               // if(pending_ch_array[x] < FD_SETSIZE) {
                    ...
                    is_fdset_empty = 0;
                    if(pending_ch_array[x] > max_fd)
                        max_fd = pending_ch_array[x];
         //       }
            }
        }
       ...
       //         if(fd_x < FD_SETSIZE) {
        ...
        if(is_fdset_empty == 1) /* still no socket to check */
            goto ctl_srv_recv;

        retval = select(max_fd + 1, &rfds, &ofds, NULL, &tv);
        if(retval == SOCKET_ERROR) {
            VR_LOG(LOG_ERROR, "Error at select(). (%d) Retrying...", WSAGetLastError());
        ...
        for(int x = 0 ; x < MAX_CONCURRENT_CHANNELS ; x++) {
            if(pending_ch_array[x] > 0) {
                if(FD_ISSET(pending_ch_array[x], &ofds) || FD_ISSET(pending_ch_array[x], &rfds)) {
                   ...
                   // non_blocking = 1;
                    //ioctlsocket(pending_ch_array[x], FIONBIO, &non_blocking);

                    r0 = send(pending_ch_array[x], &dummy, 0, 0);
...
int start_relay_conn(char *host, int port, proto_t proto, char *key, size_t key_sz) {
    ...
    if(WSAStartup(MAKEWORD(2, 2), &wsa) != 0) {
        VR_LOG(LOG_ERROR, "Error at WSAStartup...");
        return 0;
    }
    #endif
    ...
    while(1) {
        ...
        VR_LOG(LOG_DEBUG, "Starting VROUTE handshake with relay server (fd=%d)...", ctl_sock);
